...
> Обновлено 2026-09-11. Рабочая ветка `main`; промежуточная редакция теории для проверки пользователем.

## Теория: текущий редакторский этап

- Полностью переписаны 55 canonical source lessons: 001–027, 030–032, 034–036,
  061–082. Это Python, NumPy/pandas/EDA, SQL, основная математика, sklearn,
  весь Deep Learning и NLP. Вместо фрагментов — связные объяснения, исходные данные,
  вычисления, ограничения, вопросы с разбором и самостоятельная практика.
- Содержательные математические главы 028, 029, 033 сохранены с точечными исправлениями
  кода, формул, обратной связи и переходов. В 040 исправлена совместимость обозначения
  рублей с KaTeX. Полная дальнейшая редактура Classic ML ещё не выполнена.
- Для 55 новых глав заданы индивидуальные цели и длительность 25–35 минут.
  Stable lesson IDs, практические интеграции, иллюстрации и интерактивные компоненты сохранены.
- Manifest `content_revision: theory-2026-09` отделяет посещения новых разделов от старых.
  История освоения, заметки и прежние посещения не удаляются; Focus поясняет отличие редакций.
- PWA cache version: `datapath-v22-theory-editorial`. Offline snapshot пересобирается
  штатным release-snapshot; публичный Pages workflow собирает его из чистой базы.
- Проверены 141 Python-блок на синтаксис, 22 SQLite-блока исполнением, 31 автономный
  PyTorch-блок; отдельно выполнен BERT с проверкой маски дополнения.
  Связанные тесты: backend 47, frontend 32 — прошли.
- Структурная content validation: 0 ошибок, 0 предупреждений. Quality audit: 100 уроков,
  0 ошибок, 0 предупреждений, 46 необязательных рекомендаций, преимущественно о явном
  обозначении блока ошибок. Это не оценка педагогического совершенства.
- Добавлена `npm run content:math` в frontend: проверка всего canonical корпуса тем же
  KaTeX, включая управляющие символы и LaTeX вне разделителей.
- Финальная проверка формул: 100 уроков, 681 формула, 0 ошибок. `make build-web` прошёл:
  content sync, offline snapshot, TypeScript и production build. Снимок содержит все
  100 уроков, в том числе 55 с новой редакцией сцен.
- По просьбе пользователя редактура приостановлена для проверки текущего результата в PWA.
  Осталось: LLM/RAG 083–091, MLOps 092–100 и содержательное улучшение Classic ML 037–060.
  Не объявлять весь учебный корпус полностью переработанным.

Подробная редакторская память: `docs/theory-editorial-audit.md`.
Ниже — сохранённое описание работающих возможностей, не подтверждение завершённости новой теории.
...
- Service worker `datapath-v22-theory-editorial` кэширует каталоги и runner вместе с
