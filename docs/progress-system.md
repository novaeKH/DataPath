...
1. Сначала продолжить незавершённый урок (`continue_lesson`).
2. Затем следующий урок маршрута (`next_lesson`).
3. Слабые темы (`weak_skills`, до 3) — только при достаточном evidence.
...
- Свободный текст не оценивается (Фаза 6); кейсы используют только
