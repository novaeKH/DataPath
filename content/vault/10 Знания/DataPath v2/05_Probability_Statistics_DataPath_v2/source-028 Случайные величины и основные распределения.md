...
## Как выбирать распределение по механизму данных

Распределение — это модель процесса, а не форма, которую подбирают только по гистограмме. Бернулли описывает одно испытание с двумя исходами, например клик или отсутствие клика. Биномиальное распределение считает число успехов в фиксированном числе независимых испытаний с одинаковой вероятностью. Если число попыток или вероятность меняются, предпосылки нужно пересмотреть.

Пуассон часто служит моделью числа событий за интервал, когда события редки и происходят с приблизительно постоянной интенсивностью. Экспоненциальное распределение связано со временем ожидания между такими событиями. Нормальное распределение удобно для суммы многих небольших независимых влияний, но не обязано описывать доходы, время ответа или другие асимметричные величины.

Пусть вероятность покупки равна 0.2, а сайт посетили пять независимых пользователей. Ожидаемое число покупок равно (np=1), но это не означает, что покупка обязательно будет одна. Биномиальная модель распределяет вероятность между результатами от 0 до 5 и отдельно описывает разброс.

В анализе сначала называют случайную величину, её возможные значения и механизм появления, затем проверяют параметры и предпосылки. Среднее и дисперсия дополняют распределение, но не заменяют его форму и хвосты.
