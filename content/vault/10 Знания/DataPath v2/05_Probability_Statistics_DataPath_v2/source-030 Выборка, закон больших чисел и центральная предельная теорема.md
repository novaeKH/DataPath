...
## Один эксперимент, три разных объекта

Пусть нас интересует средний чек всех клиентов — это параметр генеральной совокупности. Мы берём случайную выборку из 100 клиентов и считаем её среднее — это одна оценка. Если мысленно повторить выборку много раз, получится распределение выборочных средних. Его нельзя путать с распределением отдельных чеков.

Закон больших чисел говорит, что при подходящих условиях выборочное среднее приближается к истинному при росте объёма выборки. Центральная предельная теорема описывает форму распределения нормированного среднего: оно часто становится близким к нормальному, даже если сами чеки асимметричны. Теорема не утверждает, что исходные данные становятся нормальными.

Стандартная ошибка среднего приблизительно уменьшается как (1/\sqrt{n}). Чтобы сократить её вдвое, нужно примерно в четыре раза больше независимых наблюдений. Слово «независимых» критично: тысяча снимков одних и тех же десяти клиентов не содержит столько же информации, сколько тысяча разных клиентов.

Поэтому до использования стандартных формул определяют единицу выборки, способ отбора и зависимость между строками. Временные, групповые и кластерные данные требуют разбиения и оценки неопределённости, которые сохраняют эту структуру.
