...
## Пошаговый A/B-пример

Сначала формулируют нулевую гипотезу: новая версия не меняет выбранную метрику. Затем заранее фиксируют основную метрику, единицу рандомизации, минимально значимый эффект, уровень ошибки первого рода и длительность эксперимента. Эти решения нельзя безнаказанно менять после просмотра результата.

`p-value` — вероятность получить наблюдение не менее экстремальное при условии, что нулевая гипотеза и модель теста верны. Это не вероятность того, что нулевая гипотеза верна, и не размер эффекта. Маленький эффект на огромной выборке может быть статистически значимым, но бесполезным для бизнеса.

Ошибка первого рода — объявить эффект, которого нет; ошибка второго рода — пропустить существующий эффект. Мощность равна вероятности обнаружить эффект заданного размера. Она растёт с объёмом выборки, величиной эффекта и допустимым уровнем ошибки, поэтому планирование эксперимента связывает статистику с практической стоимостью.

Если одновременно проверить двадцать метрик и выбрать единственную с `p < 0.05`, вероятность случайной находки заметно возрастёт. Нужны заранее выбранная primary metric и корректировка для семейства дополнительных проверок. Вместе с `p-value` всегда показывайте оценку эффекта и доверительный интервал.
