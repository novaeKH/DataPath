...
Пусть требуется найти города, где оплаченная выручка за месяц превысила 100 000. Сначала `WHERE` убирает неоплаченные строки, затем `GROUP BY city` создаёт группы, после чего `SUM(amount)` вычисляет показатель внутри каждой группы. Только теперь `HAVING SUM(amount) > 100000` может отфильтровать агрегированный результат.

Различие между `WHERE` и `HAVING` не стилистическое. `WHERE` отвечает на вопрос «какие исходные строки участвуют в расчёте?», а `HAVING` — «какие уже посчитанные группы оставить?». Если перенести условие оплаты в `HAVING`, запрос либо станет неверным, либо изменит смысл показателя.

До агрегации нужно назвать гранулярность входа, после неё — гранулярность результата. В таблице заказов одна строка могла означать заказ, а после `GROUP BY city, month` одна строка означает пару «город–месяц». Любой выбранный неагрегированный столбец должен быть частью этой пары; иначе непонятно, какое из многих значений вернуть.

Для проверки посчитайте общую сумму отдельно и сравните её с суммой по группам. При условной агрегации `SUM(CASE WHEN ... THEN 1 ELSE 0 END)` внимательно различайте количество строк, количество уникальных сущностей и сумму денежного показателя.
