...
`NULL` означает отсутствие известного значения, поэтому обычные сравнения дают не `TRUE` или `FALSE`, а неизвестный результат. В `WHERE` остаются только строки, для которых условие истинно; строки с неизвестным результатом отбрасываются. Отсюда правило: проверять пропуск через `IS NULL` и отдельно решать, как он должен участвовать в бизнес-логике.

`CASE` полезно читать сверху вниз как первую подходящую ветку. Условия могут пересекаться, поэтому более узкое ставят раньше более общего. Например, заказ с суммой 20 000 одновременно подходит под «больше 5 000» и «больше 10 000»; порядок веток определит метку.

При работе с датами различайте момент времени и календарный период. Фильтр `created_at < next_month_start` обычно надёжнее попытки включить последний день до `23:59:59`, особенно если столбец хранит доли секунды. Часовой пояс также является частью смысла, а не форматом отображения.

Строковые функции применяют после проверки качества исходных значений. `LOWER` и `TRIM` могут исправить технические различия, но не должны случайно объединять разные бизнес-категории. В итоговом запросе явно фиксируйте, что происходит с `NULL` в каждой ветке `CASE`, агрегате и строковом преобразовании.
