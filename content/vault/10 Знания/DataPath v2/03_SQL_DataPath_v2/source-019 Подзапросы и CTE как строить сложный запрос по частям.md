...
Сложный запрос проще строить от промежуточных таблиц. Допустим, нужно выбрать клиентов, чья месячная выручка выше среднего среди клиентов. Первый CTE `customer_revenue` приводит заказы к одной строке на клиента и считает сумму. Второй шаг вычисляет среднее уже по этим клиентским суммам. Финальный `SELECT` сравнивает величины одной гранулярности.

Если сравнить отдельный заказ со средней выручкой клиента, синтаксис может быть допустимым, но смысл окажется другим. Поэтому каждому CTE полезно дать имя, отражающее результат, и записать рядом ожидаемую единицу строки. CTE не обязан ускорять запрос; его главная учебная ценность — сделать этапы и границы агрегации видимыми.

Коррелированный подзапрос логически выполняет внутреннее условие для текущей строки внешнего запроса. Он выразителен для `EXISTS`, но на больших данных может быть сложнее для чтения и оптимизации. Часто ту же задачу яснее решить предварительной агрегацией и `JOIN`.

Проверяйте каждый CTE отдельно: число строк, уникальность ключа, диапазон показателей. Когда финальный результат неверен, такой запрос позволяет найти первый этап, на котором изменилась гранулярность или появились дубликаты.
