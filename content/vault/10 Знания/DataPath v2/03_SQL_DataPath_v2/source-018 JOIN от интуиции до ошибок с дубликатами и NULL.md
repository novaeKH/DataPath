...
Возьмём две таблицы: `orders` с несколькими заказами одного клиента и `customers`, где ожидается одна строка на клиента. Перед соединением проверяем ключи: `orders.customer_id` не обязан быть уникальным, а `customers.customer_id` обязан. Тогда `LEFT JOIN` должен сохранить число заказов и добавить к каждому данные клиента.

Если в справочнике клиентов один ключ встречается дважды, каждый соответствующий заказ размножится. Запрос выполнится без ошибки, но сумма выручки после соединения вырастет. Поэтому качество `JOIN` проверяют не только просмотром колонок: сравнивают число строк, сумму ключевого показателя и количество уникальных ключей до и после операции.

Тип соединения выбирают по вопросу. `INNER JOIN` оставляет только найденные пары; `LEFT JOIN` сохраняет все строки левой таблицы и ставит `NULL` там, где соответствия нет. Пропавшая строка после `INNER JOIN` может означать как ожидаемое отсутствие справочника, так и грязный ключ с пробелом или другим типом.

Если связь действительно многие-ко-многим, размножение может быть правильным, но его нужно осознанно учитывать в последующей агрегации. Главный вопрос перед `JOIN`: что означает одна строка в каждой таблице и сколько соответствий допустимо для одного ключа.
