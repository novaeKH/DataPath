...
Оконная функция отличается от `GROUP BY` тем, что не сворачивает строки. Например, у каждого заказа можно сохранить исходные колонки и одновременно показать место заказа по сумме внутри клиента. `PARTITION BY customer_id` создаёт независимое окно для каждого клиента, а `ORDER BY amount DESC` задаёт порядок внутри окна.

Для временной последовательности порядок должен быть однозначным. Если у двух событий одинаковое время, добавьте второй ключ, например `event_id`; иначе `LAG` или `ROW_NUMBER` могут возвращать нестабильный результат. `LAG(amount)` берёт значение из предыдущей строки в заданном порядке, но не гарантирует, что событие произошло ровно днём ранее.

У накопительной суммы важен frame. Конструкция `ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW` явно означает все физические строки от начала окна до текущей. Значение frame по умолчанию зависит от СУБД и типа порядка, поэтому в аналитическом запросе безопаснее записывать его явно.

Порядок рассуждения: определить одну строку результата, выбрать группы через `PARTITION BY`, задать строгий порядок, затем выбрать функцию и границы окна. После этого вручную просчитайте один небольшой partition из трёх-четырёх строк — так быстрее всего обнаружить ошибку в направлении сортировки или frame.
