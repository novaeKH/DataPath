...
## Сквозной пример: подготовка идентификаторов

Пусть поток содержит миллион `user_id`, часть из них повторяется. Если нужно сохранить исходный порядок всех событий, подходит `list`. Если нужно быстро отвечать «видели ли мы пользователя раньше?», рядом поддерживают `set`: проверка принадлежности в среднем не требует просмотра всех элементов.

Для подсчёта частот естественен `dict`, где ключ — идентификатор, значение — число появлений; `Counter` даёт готовую реализацию того же паттерна. Если требуется убрать повторы и сохранить порядок первого появления, можно пройти список один раз, проверяя `set`, и добавлять новые элементы в результирующий `list`.

`tuple` выбирают не потому, что он «быстрее списка во всех случаях», а когда фиксированная структура не должна менять длину. Ключ словаря обязан быть hashable: его хэш и равенство должны оставаться согласованными, пока ключ находится в таблице.

Comprehension удобен для одного ясного преобразования или фильтра. Когда внутри появляются несколько ветвей, побочные эффекты и сложная обработка ошибок, обычный цикл читается лучше. Выбор коллекции начинается с допустимых операций и смысла данных, а уже затем — с оценки времени и памяти.
