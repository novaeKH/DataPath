...
## Сквозной пример: настраиваемое преобразование

Пусть функция должна обрезать значения по заданной границе. Явные параметры `values` и `limit` делают контракт понятным; keyword-only параметр помогает избежать вызова с непонятным числом. Внутри функция создаёт результат и не меняет вход — такое преобразование легче тестировать и повторно использовать в pipeline.

Если одна и та же граница используется многократно, фабрика может вернуть closure: внешняя функция получает `limit`, внутренняя запоминает его и применяет к новым данным. Замыкание хранит ссылку на переменную из внешней области, а не текстовое значение, поэтому late binding особенно важно понимать при создании функций в цикле.

Decorator добавляет поведение вокруг вызова, например измеряет время или пишет лог. Запись `@timer` эквивалентна присваиванию `train = timer(train)`. `functools.wraps` сохраняет имя и документацию исходной функции, что важно для отладки и инструментов.

Так scope, closure и decorator складываются в одну модель: функции являются объектами, могут передаваться и возвращаться, а поиск свободного имени идёт по LEGB. `*args` и `**kwargs` стоит применять только там, где гибкость действительно важнее явного интерфейса.
