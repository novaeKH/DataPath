...
## Сквозной пример: обработка большого файла

Если файл не помещается в память, функция-генератор может читать его построчно, пропускать некорректные записи и выдавать по одной очищенной строке. Следующий генератор фильтрует нужные события, а третий преобразует их в признаки. Данные проходят через цепочку только по запросу потребителя.

Цикл `for` сначала вызывает `iter`, затем многократно `next`, пока итератор не сообщит `StopIteration`. Генератор реализует этот протокол автоматически: `yield` возвращает значение и сохраняет точку продолжения. Поэтому он ленивый и одноразовый — после полного обхода состояние исчерпано.

Память экономится потому, что одновременно хранится лишь небольшой текущий фрагмент. Но вычисления не исчезают, а повторный проход невозможен без создания нового генератора. Если нужны случайный доступ, многократный анализ или векторные операции над небольшим массивом, материализация в список или NumPy может быть разумнее.

Проверяя pipeline, возьмите первые несколько элементов через `itertools.islice`, но помните: это тоже потребляет их. Явно решайте, кто владеет итератором и сколько раз данные должны быть прочитаны.
