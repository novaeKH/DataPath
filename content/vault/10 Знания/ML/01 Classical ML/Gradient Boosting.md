...
- табличные данные и задача качества «из коробки»;
- приемлемая вычислительная стоимость обучения;
- нужны feature importance и хорошая работа со смешанными типами признаков;
- НЕ использовать, если важна максимальная интерпретируемость (лучше дерево/линейная модель) или обучение должно быть мгновенным на очень больших данных без GPU/оптимизаций.
...
Компонент `boosting-residuals-lab`: шаги boosting — видно, как новая модель обучается на остатках предыдущей и последовательно исправляет ошибки; параметры — learning rate и глубина деревьев.

## Сравнение с Random Forest

- GB строит деревья последовательно, каждое исправляет ошибки предыдущего; RF усредняет независимые деревья;
- GB обычно даёт ниже bias (и выше риск overfit); RF стабильнее и проще настраивается;
- GB чувствителен к learning rate и глубине; RF — к числу деревьев и max_features.
...
- [[Ensemble Comparison]] — когда boosting, а когда bagging.
