...
CODE_EXERCISES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "track": "sklearn",
        "title": "Первая линейная регрессия",
        "difficulty": "foundation",
        "estimated_minutes": 15,
        "prompt": "Обучите LinearRegression на X_train и y_train, получите прогнозы для X_test и сохраните RMSE в переменную rmse.",
        "starter_code": "model = ...\nmodel.fit(...)\npred = ...\nrmse = ...",
        "hint": "Сначала создайте estimator. fit получает только обучающую часть; predict — только признаки тестовой части. Для RMSE используйте root_mean_squared_error.",
        "skill_id": "ml.linear_regression",
        "required": [
            "linearregression()",
            ".fit(x_train,y_train)",
            ".predict(x_test)",
    ...
    {
        "id": "sklearn.regression-debug",
        "track": "sklearn",
        "title": "Исправьте нечестную оценку регрессии",
        "difficulty": "core",
        "estimated_minutes": 15,
        "prompt": "Код обучает и оценивает модель на одних строках. Сделайте train/test split, обучите модель только на train и посчитайте MAE только на test.",
        "starter_code": "X = df.drop(columns='target')\ny = df['target']\nmodel = LinearRegression().fit(X, y)\npred = model.predict(X)\nmae = mean_absolute_error(y, pred)",
        "hint": "Первый шаг — отделить тестовые строки до fit. Названия X_train, X_test, y_train, y_test помогут не смешать роли данных.",
        "skill_id": "ml.validation_split",
        "required": [
            "train_test_split(",
            ".fit(x_train,y_train)",
            ".predict(x_test)",
