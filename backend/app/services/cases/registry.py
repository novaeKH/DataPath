...
class Case:
    ...
    def evaluate(self, answers: dict[str, Any], mode: str = "standard") -> dict[str, Any]:
        ...
        return {
            ...
            "conclusion": self.conclusion,
            # Показывается только после отправки ответов: до попытки скрытые
            # проблемы датасета не раскрываются в spec().
            "expected_problems": list(self.expected_problems),
        }
...
def _domain_cases() -> list[Case]:
    ...
    return [
        ...
        Case(
            ...
            title="Спрос на велосипеды: первая регрессионная модель",
            content_id="case.ml.bike-demand-regression",
            description="Провести аудит реальных данных UCI, убрать утечку, выбрать временную проверку, построить базовый прогноз и Linear Regression, затем разобрать ошибки.",
            practice_kind="module-case",
            lesson_ids=[
                "lesson.data-analysis.09",
            ...
            intro=(
                "Городская служба проката планирует число доступных велосипедов на каждый час. "
                "Используйте UCI Bike Sharing (17 389 часовых наблюдений, 2011–2012). Цель cnt — "
                "число аренд. До моделирования зафиксируйте момент прогноза и проведите аудит: "
                "форма таблицы, типы, пропуски, дубликаты, подозрительные столбцы и порядок времени."
            ),
            conclusion=(
                "Эталонный путь: прогнозируем cnt до наступления часа; удаляем casual, registered "
                "и технический instant; сортируем по dteday+hr и отделяем более поздний период; "
                "сравниваем среднее обучающей части с leakage-safe LinearRegression pipeline; "
                "читаем MAE, RMSE и R² вместе и завершаем работу анализом крупнейших остатков по "
                "часу, погоде, рабочему дню и сезону."
            ),
            expected_problems=[
                "casual и registered в сумме точно образуют cnt и недоступны до завершения часа — это прямая утечка",
                "случайное перемешивание строк смешивает прошлое и будущее; нужна временная проверка",
                "instant — технический идентификатор и скрытый указатель порядка, а не содержательный признак",
                "season, mnth, hr, weekday и weathersit закодированы числами, но являются категориальными",
                "temp и atemp сильно связаны, поэтому отдельные коэффициенты могут быть нестабильны",
                "пропусков нет, но это не отменяет дубликаты, неверные типы, утечку и проверку диапазонов",
                "почасовой спрос нелинеен и имеет пики; линейная модель без преобразований систематически ошибается",
                "самые крупные ошибки могут концентрироваться в редкой погоде и часах пик",
            ],
            questions=[
                CaseQuestion(
                    id="q1",
                    type="single",
                    topic="theory",
                    prompt="Какой контракт задачи соответствует планированию числа велосипедов до начала каждого часа?",
                    options=[
                        "Объект — час, цель — cnt, признаки доступны до начала часа",
                        "Объект — пользователь, цель — registered после завершения часа",
                        "Объект — день, цель — casual + registered из того же часа",
                        "Объект — строка без определения момента прогноза",
                    ],
                    correct=0,
                    hint="Сначала назовите одну строку, прогнозируемое число и момент доступности признаков.",
                    explanation="Честный контракт фиксирует объект 'час', целевую переменную cnt и момент прогноза до наступления этого часа.",
                ),
                CaseQuestion(
                    id="q2",
                    type="multiple",
                    topic="data",
                    prompt="До просмотра результатов модели какие проблемы разумно искать в этом наборе?",
                    options=[
                        "Пропуски и дубликаты",
                        "Утечку из столбцов, известных только после часа",
                        "Числовые коды категорий",
                        "Временную зависимость и изменение спроса",
                        "Только баланс классов",
                    ],
                    correct=[0, 1, 2, 3],
                    hint="Это регрессия с временным порядком и смесью технических, категориальных и числовых столбцов.",
                    explanation="Аудит проводят до обучения: проверяют качество данных, доступность признаков, семантические типы и время. Баланс классов к числовой цели cnt не относится.",
                ),
                CaseQuestion(
                    id="q3",
                    type="order",
                    topic="apply",
                    prompt="Упорядочьте первые шаги исследования до обучения модели.",
                    options=[
                        "Зафиксировать объект, цель и момент прогноза",
                        "Посмотреть shape, столбцы, типы, пропуски и дубликаты",
                        "Проверить диапазоны, время и возможную утечку",
                        "Только затем отделить train и test",
                    ],
                    correct=[0, 1, 2, 3],
                    hint="Контракт определяет, что считать утечкой; аудит предшествует разбиению, а разбиение — fit.",
                    explanation="Без контракта невозможно понять доступность признаков. После аудита выбирают схему проверки и только затем обучают модель.",
                ),
                CaseQuestion(
                    id="q4",
                    type="multiple",
                    topic="data",
                    prompt="Какие столбцы нельзя давать модели, предсказывающей cnt до начала часа?",
                    options=[
                        "casual",
                        "registered",
                        "temp",
                        "weathersit, если прогноз погоды доступен",
                        "cnt как входной признак",
                    ],
                    correct=[0, 1, 4],
                    hint="По описанию UCI cnt = casual + registered. Спросите, когда становятся известны эти счётчики.",
                    explanation="casual и registered — части целевой переменной, известные после часа; cnt нельзя дублировать во входе. Погода допустима только при честном источнике прогноза.",
                ),
                CaseQuestion(
                    id="q5",
                    type="single",
                    topic="interpret",
                    prompt="Какое разбиение лучше воспроизводит применение модели в будущем?",
                    options=[
                        "Случайно перемешать все часы",
                        "Обучить на более ранних датах, проверить на более поздних",
                        "Выбрать строки с самой большой cnt в test",
                        "Оценить на обучающей части",
                    ],
                    correct=1,
                    hint="В эксплуатации модель всегда переносится из прошлого в будущее.",
                    explanation="Временное разбиение сохраняет порядок применения и лучше обнаруживает изменение спроса между периодами.",
                ),
                CaseQuestion(
                    id="q6",
                    type="multiple",
                    topic="data",
                    prompt="Какие признаки следует рассматривать как категории, хотя в CSV они записаны числами?",
                    options=["hr", "season", "weekday", "weathersit", "temp"],
                    correct=[0, 1, 2, 3],
                    explanation="Час, сезон, день недели и тип погоды — коды состояний. temp — нормированная числовая температура.",
                ),
                CaseQuestion(
                    id="q7",
                    type="single",
                    topic="data",
                    prompt="UCI указывает, что пропусков нет. Какой вывод корректен?",
                    options=[
                        "Аудит завершён",
                        "Можно не проверять дубликаты и диапазоны",
                        "Пропуски не проблема, но типы, утечка, время, ID и диапазоны всё ещё нужно проверить",
                        "Данные гарантированно готовы к production",
                    ],
                    correct=2,
                    explanation="Отсутствие пропусков закрывает только одну проверку и не гарантирует корректный эксперимент.",
                ),
                CaseQuestion(
                    id="q8",
                    type="numeric",
                    topic="metrics",
                    prompt="Базовый прогноз равен среднему train: 189 аренд. Для трёх test-часов cnt = [100, 220, 247]. Чему равна MAE?",
                    correct=59.33,
                    numeric_tolerance=0.02,
                    hint="Найдите |y - 189| для каждого часа и усредните три числа.",
                    explanation="Абсолютные ошибки 89, 31 и 58; MAE = 178 / 3 ≈ 59.33 аренды.",
                ),
                CaseQuestion(
                    id="q9",
                    type="single",
                    topic="metrics",
                    prompt="Почему в отчёте полезно показать и MAE, и RMSE?",
                    options=[
                        "Они всегда одинаковы",
                        "MAE даёт типичную ошибку в арендах, RMSE сильнее реагирует на редкие крупные промахи",
                        "RMSE измеряется в квадрате аренд",
                        "MAE применима только к классификации",
                    ],
                    correct=1,
                    explanation="Обе метрики имеют единицы цели, но RMSE сильнее подчёркивает часы с тяжёлыми ошибками.",
                ),
                CaseQuestion(
                    id="q10",
                    type="order",
                    topic="apply",
                    prompt="Упорядочьте честный путь от данных к оценке Linear Regression.",
                    options=[
                        "Временное разбиение",
                        "ColumnTransformer внутри Pipeline",
                        "fit только на train",
                        "predict и метрики на test",
                        "анализ крупнейших остатков",
                    ],
                    correct=[0, 1, 2, 3, 4],
                    explanation="Разбиение делают до обучаемой предобработки; оценку завершают разбором объектов, а не одной цифрой.",
                ),
                CaseQuestion(
                    id="q11",
                    type="multiple",
                    topic="interpret",
                    prompt="Что проверить среди часов с самой большой абсолютной ошибкой?",
                    options=[
                        "Час суток и рабочий день",
                        "Погоду и сезон",
                        "Систематическое завышение или занижение",
                        "Только общий R² ещё раз",
                        "Необычные значения и возможные события",
                    ],
                    correct=[0, 1, 2, 4],
                    explanation="Анализ ошибок ищет сегменты и механизм провала. Повтор общей метрики не объясняет конкретные промахи.",
                ),
                CaseQuestion(
                    id="q12",
                    type="single",
                    topic="overfitting",
                    prompt="Модель постоянно занижает утренний и вечерний пики. Какой вывод наиболее осторожен?",
                    options=[
                        "Линейная регрессия сломана",
                        "Почасовая зависимость нелинейна; нужны корректные признаки/взаимодействия и сравнение с более гибкой моделью на том же split",
                        "Нужно добавить casual и registered",
                        "Следует выбирать модель по train RMSE",
                    ],
                    correct=1,
                    explanation="Пики — ожидаемый сигнал неверной формы. Сначала улучшают представление времени и честно сравнивают модели, не возвращая утечку.",
                ),
            ],
        ),
        Case(
